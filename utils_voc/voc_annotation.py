...
def run_voc_annotation(VOCdevkit_path, classes_path):
    annotation_mode     = 0
    #-------------------------------------------------------------------#
    #   必须要修改，用于生成2007_train.txt、2007_val.txt的目标信息
    #   与训练和预测所用的classes_path一致即可
    ...
    if annotation_mode == 0 or annotation_mode == 2:
        ...
        for image_set in VOCdevkit_sets:
            ...
            txt_path = os.path.join(INDEX_DIR, '%s.txt'%(image_set))
            list_file = open(txt_path, 'w', encoding='utf-8')
            for image_id in image_ids:
                file_path = VOCdevkit_path.replace('\\', '/')
                
                list_file.write('%s/JPEGImages/%s.jpg'%(file_path, image_id))
...
if __name__ == "__main__":
    run_voc_annotation(DATASET_NAME, CLASSES_PATH)
